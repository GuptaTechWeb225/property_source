...
class MyProfileController extends Controller
{
    ...
    {
        $data['title'] = _trans('common.my_profile');
    ...
    public function update(ProfileUpdateRequest $request)
    {
        ...
        if ($result) {
            return redirect()->route('my.profile')->with('success', _trans('alert.profile_updated_successfully'));
        }
        return redirect()->route('my.profile')->with('danger', _trans('alert.something_went_wrong_please_try_again'));
    }
    ...
    public function passwordUpdateStore(PasswordUpdateRequest $request)
    {
        if (Hash::check($request->current_password, Auth::user()->password)) {
            ...
            if ($result) {
                return redirect()->route('passwordUpdate')->with('success', _trans('alert.password_updated_successfully'));
            }
            return redirect()->route('passwordUpdate')->with('danger', _trans('alert.something_went_wrong_please_try_again'));
        } else {
