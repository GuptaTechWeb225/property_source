...
class InstallRepository
{
    ...
    public function validateDatabase($params)
    {
        try {
            ...
            if (!$link) {
                throw new ValidationException(['message' => _trans('common.Connection Not Established')]);
            }

            // Attempt to select the database
            $select_db = mysqli_select_db($link, $db_database);
            if (!$select_db) {
                throw new ValidationException(['message' => _trans('common.DB Not Found')]);
            }

            // Check for existing tables only if force_migrate is not set
            if (!gbv($params, 'force_migrate')) {
                $count_table_query = mysqli_query($link, "SHOW TABLES");
                $count_table = mysqli_num_rows($count_table_query);

                if ($count_table) {
                    throw new ValidationException(['message' => _trans('common.Existing Table In Database')]);
                }
            }

            // Log successful database validation
            ...
            // Log the exception
            Log::error('Database validation error: ' . $th->getMessage());
    ...
    public function migrateDB()
    {
        ...
        try {
            ini_set('max_execution_time', -1);
            $sql = base_path('installer/db/landlord.sql');
        } catch (Throwable $e) {
