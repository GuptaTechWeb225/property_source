        ...
        <div class="padding-left-top">
            <div class="bg-white w-75 rounded" step-count="5">
                <div class="text-title p-3 text-center text-white">
                    <h3>{{ _trans('service::install.admin_setup') }}</h3>
                </div>

                <form class="pb-3 content-body" method="post" action="{{ route('service.user') }}" id="content_form">
                    <div class="mb-3 px-5 pt-5">
                        <label class="form-label"><b>{{ _trans('service::install.email') }}<span
                                    class="star">*</span></b></label>
                        <input type="email" class="form-control" name="email" id="email" required="required"
                            placeholder="{{ _trans('service::install.email') }}">
                    </div>
                    <div class="mb-3 px-5">
                        <label class="form-label" for="password"><b>{{ _trans('service::install.password') }}<span
                                    class="star">*</span></b></label>
                        <input type="password" class="form-control" name="password" id="password"
                            placeholder="{{ _trans('service::install.password') }}" required>

                    </div>
                    <div class="mb-3 px-5 pb-3">
                        <label class="form-label"
                            for="password_confirmation"><b>{{ _trans('service::install.password_confirmation') }}<span
                                    class="star">*</span></b></label>
                        <input type="password" class="form-control" name="password_confirmation" id="password_confirmation"
                            required placeholder="{{ _trans('service::install.password_confirmation') }}"
                    ...
                    <div class="px-5 pb-4 d-flex flex-column justify-content-center align-items-start gap-3">
                        <button type="submit"
                            class="btn color btn-primary px-5 py-3 align-items-start follow-next-step submit bc-color">{{ _trans('service::install.ready_to_go') }}
                            »</button>
                        <button type="button"
                            class="btn color btn-primary px-5 py-3 align-items-start follow-next-step submitting bc-color"
                            disabled style="display:none">{{ _trans('service::install.submitting') }} »</button>
...
@push('js')
    <script>
        ...
        $(document).ready(function() {
            setTimeout(function() {
                $('.preloader h2').html(
                    'Installing The System. <br> This may take some time. Be patient. Do not refresh or close your browser'
                    )
            }, 2000);
        })
    </script>
