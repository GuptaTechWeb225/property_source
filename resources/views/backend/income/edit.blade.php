        ...
        <div class="table-content table-basic mt-20">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="mb-0">{{ _trans('common.Income Edit') }}</h4>
                </div>
                <div class="card-body ot-card">
                    <form action="{{ route('income.update',$income->id) }}" method="post" class="row" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        <x-forms.select label="Account" name="account_id" :required="true" value="{{ @old('account_id') }}">
                            ...
                            @foreach($accounts as $account)
                                <option value="{{ $account->id }}" {{ $account->id == $income->account_id ? 'selected' : '' }}>
