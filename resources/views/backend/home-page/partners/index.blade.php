        ...
        <div class="table-content table-basic mt-20">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="mb-0">{{ _trans('landlord.all partners') }}</h4>
                    @if (hasPermission('partners_create'))
                        <a href="{{ route('home.partners.create') }}" class="btn btn-lg ot-btn-primary">
                        ...
                        <table class="table table-bordered ot-table-bg language-table">
                            <thead class="thead">
                                <tr>
                                    ...
                                    <th class="purchase">{{ _trans('landlord.logo') }}</th>
                                    <th class="purchase">{{ _trans('landlord.status') }}</th>
                                    @if (hasPermission('partners_update') ||
                                        hasPermission('partners_delete'))
                                        <th class="action">{{ _trans('landlord.action') }}</th>
                                    @endif
                                </tr>
                            </thead>
                            <tbody class="tbody">
                                @forelse ($data['partners'] as $key => $row)
                                <tr id="row_{{ $row->id }}">
                                        ...
                                        <td class="action">
                                            <div class="dropdown dropdown-action">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    @if (hasPermission('partners_update'))
                                                        <li>
                                                            <a class="dropdown-item"
                                                                href="{{ route('home.partners.edit', $row->id) }}"><span
                                                                    class="icon mr-8"><i
                                                    ...
                                                    @endif
                                                    @if ($row->code != 'en')
                                                        @if (hasPermission('partners_delete'))
                                                            <li>
                                                                <a class="dropdown-item" href="javascript:void(0);"
                                                                    onclick="delete_row('homepage/partners/delete', {{ $row->id }})">
