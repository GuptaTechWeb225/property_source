...
<div class="page-content">
                ...
                <h4 class="bradecrumb-title mb-1">{{ $data['title'] }}</h1>
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item" aria-current="page"><a
                                href="{{ route('cities.index') }}">{{ _trans('landlord.cities') }}</a>
                        </li>
    ...
    <div class="card ot-card">
        <div class="card-body">
            <form action="{{ route('cities.store') }}" enctype="multipart/form-data" method="post" id="visitForm">
                @csrf
                <div class="row mb-3">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        {{-- Status --}}
                        <label for="validationServer04" class="form-label">{{ _trans('landlord.State') }} <span
                                class="fillable">*</span></label>
                    ...
                    <div class="col-md-6 mb-3" id="countryDiv">
                        <label for="countrySelect" class="form-label">{{ _trans('landlord.Country') }} <span
                                class="fillable">*</span></label>
                        <select
                            class="nice-select niceSelect bordered_style wide @error('country_id') is-invalid @enderror"
                            name="country_id" id="countrySelect" aria-describedby="countryFeedback">
                            @foreach ($data['countries'] as $country)
                            <option value="{{ $country->id }}">{{ $country->name }}</option>
                            @endforeach
                        </select>
