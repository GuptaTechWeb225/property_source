        ...
        <div class="table-content table-basic mt-20">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="mb-0">{{ _trans('landlord.Facility Type') }}</h4>
                    @if (hasPermission('property_facility_type_create'))
                        <a href="{{ route('properties.facilityType.create') }}" class="btn btn-lg ot-btn-primary">
                        ...
                        <table class="table table-bordered ot-table-bg category-table">
                            <thead class="thead">
                                <tr>
                                    ...
                                    @if (hasPermission('property_facility_type_update') || hasPermission('property_facility_type_delete'))
                                        <th class="action">{{ _trans('landlord.Action') }}</th>
                                    @endif
                                </tr>
                            </thead>
                            <tbody class="tbody">
                                @forelse ($data['facilityType'] as $key => $row)
                                    <tr id="row_{{ $row->id }}">
                                            ...
                                            <td class="action">
                                                <div class="dropdown dropdown-action">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end">
                                                        @if (hasPermission('property_facility_type_update'))
                                                            <li>
                                                                <a class="dropdown-item"
                                                                    href="{{ route('properties.facilityType.edit', $row->id) }}">
                                                        ...
                                                        @endif
                                                        @if (hasPermission('property_facility_type_delete'))
                                                            <li>
                                                                <a class="dropdown-item" href="javascript:void(0);"
                                                                    onclick="delete_row('property/facility-types/delete', {{ $row->id }})">
