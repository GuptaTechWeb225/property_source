    ...
    <div class="page-content">
        ...
        <div class="card ot-card">
            {{-- <div class="card-header">
                <h4>{{ _trans('common.edit_terms') }}</h4>
            </div> --}}
            <div class="card-body">
                <form action="{{ route('languages.update.terms', @$data['language']->code) }}" enctype="multipart/form-data"
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-12 mb-3">
                            <label for="validationServer04" class="form-label">{{ _trans('common.module') }}</label>
                            ...
                            <select class="form-select ot-input @error('lang_module') is-invalid @enderror change-module"
                                ...
                                @foreach ($lans as $language)
                                    @php
                                        $file = explode('.', $language);
                                        $file_name = $file[0];
                                    @endphp
                                    <option value="{{ $file_name }}" {{ $file_name === 'common' ? 'selected' : '' }}>
                                        {{ ucwords($file_name) }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <label for="exampleDataList" class="form-label ">{{ _trans('common.term') }}</label>
                                </div>
                                <div class="col-md-6">
                                    <label for="exampleDataList"
                                        class="form-label ">{{ _trans('common.translated_language') }}</label>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-md-6 translated_language">
                                        <input class="form-control ot-input" list="datalistOptions" id="exampleDataList"
                                            placeholder="{{ _trans('common.translated_language') }}"
                                            name="{{ $key }}" value="{{ $row }}">
                                    </div>
                                </div>
                            @endforeach
