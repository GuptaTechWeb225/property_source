...
if (!function_exists('generateUniqueString')) {
    function generateUniqueString($limit)
    {
        // Generate a unique ID, and create a SHA-1 hash of it
        $uniqueString = sha1(uniqid('' . mt_rand(), true)); // Create a unique string

        // Convert the hash into base 36 (using base_convert for more compact representation)
        $slug = substr(base_convert($uniqueString, 16, 36), 0, $limit);
