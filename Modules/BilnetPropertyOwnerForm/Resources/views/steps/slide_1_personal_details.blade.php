            ...
            <div id="slide1" class="form-slide active slide">
                ...
                <div class="form-group">
                    ...
                    <div class="radio-group">
                        <label>
                            <input type="radio" name="nationality" value="ghanian" checked> Ghanian
                        </label>
                        <label>
                            <input type="radio" name="nationality" value="foreigner" id="foreignerNationality"
                                required>
                            Foreigner
                        </label>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div id="dualCitizenshipCountries" class="conditional-field hidden">
                        ...
                        <input type="hidden" name="dual_citizenship_countries" id="dual_citizenship_countries">
                    </div>
                </div>
                <div class="hr"></div>
                ...
                <div class="form-group">
                    ...
                    <div class="radio-group">
                        <label for="id_type_gh">
                            <input type="radio" name="id_type" id="id_type_gh" value="Ghana Card" checked>
                        ...
                        <label for="id_type_fp">
                            <input type="radio" name="id_type" id="id_type_fp" value="Foreigner Passport">
                            Foreigner
                            Passport
                        </label>
                        <label for="id_type_ovi">
                            <input type="radio" name="id_type" id="id_type_ovi" value="Other Valid Id"> Other
                            Valid Id
                        </label>
                        <label for="id_type_nin">
                            <input type="radio" name="id_type" id="id_type_nin" value="NIN for Nigerians"> NIN for
                            Nigerians
                        </label>
                    </div>
                    <a href="#" onclick="getVerifiedDocumentNo('id_card')">
                        <h6> (Click here to get your Ghana Card and Passport also if you are foreigner or Ghanaian)</h6>
                    </a>
                </div>
                        ...
                        <div class="card-grid">
                            ...
                            <div class="card" data-type="front">
                                ...
                                <input type="file" accept="image/png, image/jpeg, image/webp" capture="environment" style="display: none;"
                                    id="id_attachment_card_front" data-multiple="false">
                                <input type="hidden" class="store_file original_path"
                                    name="id_attachment_card_front">
                            ...
                            <div class="card" data-type="back">
                                ...
                                <button class="upload-btn" type="button">Upload Image</button>
                                <input type="file" accept="image/png, image/jpeg, image/webp" capture="environment" style="display: none;"
                                    id="id_attachment_card_back" data-multiple="false">
                                <input type="hidden" class="store_file original_path"
                                    name="id_attachment_card_back">
                                <div class="loader" style="display: none;">Uploading...</div>
                                <img class="preview" style="display: none; max-width: 100%; margin-top: 10px;" />
                            </div>

                            <!-- Front of Card Holding -->
                            <div class="card" data-type="front-holding">
                                <h3>Front of card (Holding)</h3>
                                ...
                                <input type="file" accept="image/png, image/jpeg, image/webp" capture="user" style="display: none;"
                                    id="id_attachment_card_front_hold" data-multiple="false">
                                <input type="hidden" class="store_file original_path"
                                    name="id_attachment_card_front_hold">
                                <div class="loader" style="display: none;">Uploading...</div>
                                <img class="preview" style="display: none; max-width: 100%; margin-top: 10px;" />
                            </div>

                            <!-- Back of Card Holding -->
                            <div class="card" data-type="back-holding">
                                <h3>Back of card (Holding)</h3>
                                <p>Click to open camera to snap live image of you holding back of ID to your chest</p>
                                <button class="upload-btn" type="button">Take Photo</button>
                                <input type="file" accept="image/png, image/jpeg, image/webp" capture="environment" style="display: none;"
                                    id="id_attachment_card_back_hold" data-multiple="false">
                                <input type="hidden" class="store_file original_path"
                                    name="id_attachment_card_back_hold">
                                <div class="loader" style="display: none;">Uploading...</div>
                                <img class="preview" style="display: none; max-width: 100%; margin-top: 10px;" />
                            </div>
                        </div>
                ...
                <script>
                    $(document).ready(function() {
                        function handleGenderChange(value) {
                            if (value === 'other') {
                                $('#genderOtherField').show();
                                $('#genderOtherField input[name="gender_other"]').prop('required', true);
                            } else {
                                $('#genderOtherField').hide();
                                $('#genderOtherField input[name="gender_other"]').prop('required', false);
                            }
                        }

                        // On load, check the currently selected gender
                        const selectedGender = $('#slide1 input[name="gender"]:checked').val();
                        handleGenderChange(selectedGender);

                        // On change, update based on the selected value
                        $('#slide1 input[name="gender"]').on('change', function(e) {
            ...
            @push('scripts')
                {{-- <script>
                    $(document).ready(function() {
                        // Function to handle the visibility of the countries dropdown
                        function handleDualCitizenshipChange(isChecked) {
                            if (isChecked) {
                                $('#dualCitizenshipCountries').show();
                                $('#dual_citizenship_countries').prop('required', true);
                            } else {
                                $('#dualCitizenshipCountries').hide();
                                $('#dual_citizenship_countries').prop('required', false);
                            }
                        }

                        // On load, check if the "dual_citizenship" checkbox is checked
                        const isDualCitizenshipChecked = $('#dualCitizenship').prop('checked');
                        handleDualCitizenshipChange(isDualCitizenshipChecked);

                        // On change, update based on the checkbox status
                        $('#dualCitizenship').on('change', function(e) {
                            handleDualCitizenshipChange(e.target.checked);
                        });

                        // Handle the country selection change
                        function handleCountrySelection() {
                            const selectedCountry = $('#countryDropdown').val();
                            if (selectedCountry && !$('#selectedCountries .selected-country[data-value="' + selectedCountry +
                                    '"]').length) {
                                const countryElement =
                                    `<div class="selected-country" data-value="${selectedCountry}">${selectedCountry} <button class="remove-country" type="button">Remove</button></div>`;
                                $('#selectedCountries').append(countryElement);
                            }
                        }

                        // On country selection change, add the selected country to the list
                        $('#countryDropdown').on('change', function() {
                            handleCountrySelection();
                        });

                        // Remove country from the selected list
                        $('#selectedCountries').on('click', '.remove-country', function() {
                            $(this).parent().remove();
                        });
                    });
                </script> --}}
